...
@section('title', 'History Bagi Hasil')
      ...
      <div class="flex items-center">
        ...
        <div class="ml-5 w-0 flex-1">
          <dl>
            ...
            <dd class="text-lg font-medium text-gray-900">Rp {{ number_format($stats['total_revenue'] ?? 0, 0, ',', '.') }}</dd>
          </dl>
        </div>
      </div>
    </div>
  ...
  <div class="bg-white overflow-hidden shadow rounded-lg">
    <div class="p-5">
      <div class="flex items-center">
        ...
        <div class="ml-5 w-0 flex-1">
          <dl>
            <dt class="text-sm font-medium text-gray-500 truncate">Bagian Admin</dt>
            <dd class="text-lg font-medium text-gray-900">Rp {{ number_format($stats['admin_share'] ?? 0, 0, ',', '.') }}</dd>
          </dl>
        </div>
      </div>
    </div>
  ...
  <div class="bg-white overflow-hidden shadow rounded-lg">
    <div class="p-5">
      <div class="flex items-center">
        ...
        <div class="ml-5 w-0 flex-1">
          <dl>
            <dt class="text-sm font-medium text-gray-500 truncate">Bagian Owner</dt>
            <dd class="text-lg font-medium text-gray-900">Rp {{ number_format($stats['owner_share'] ?? 0, 0, ',', '.') }}</dd>
          </dl>
        </div>
      </div>
    </div>
...
<div class="bg-white shadow rounded-lg mb-6">
  <div class="px-4 py-5 sm:p-6">
    <form method="GET" action="{{ route('admin.bagi-hasil.index') }}" class="grid grid-cols-1 gap-6 sm:grid-cols-4">
      <div>
        <label for="owner" class="block text-sm font-medium text-gray-700">Owner</label>
        <select id="owner" name="owner" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
          <option value="">Semua Owner</option>
          @if(isset($owners))
          @foreach($owners as $owner)
          <option value="{{ $owner->id }}" {{ request('owner') == $owner->id ? 'selected' : '' }}>{{ $owner->nama }}</option>
          @endforeach
          @endif
        </select>
      </div>
...
<div class="bg-white shadow overflow-hidden sm:rounded-md">
  <div class="px-4 py-5 sm:px-6">
    <h3 class="text-lg leading-6 font-medium text-gray-900">History Bagi Hasil</h3>
    <p class="mt-1 max-w-2xl text-sm text-gray-500">Riwayat pembagian hasil revenue antara admin dan owner.</p>
  </div>

  @if(isset($bagiHasils) && $bagiHasils->count() > 0)
...
<div class="mt-8 bg-white shadow rounded-lg">
  <div class="px-4 py-5 sm:p-6">
    <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">Summary Bagi Hasil</h3>
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
      <div class="bg-blue-50 rounded-lg p-4">
        <div class="flex items-center">
          ...
          <div class="ml-3">
            <p class="text-sm font-medium text-blue-800">Bulan Ini</p>
            <p class="text-lg font-semibold text-blue-900">Rp {{ number_format($summary['this_month'] ?? 0, 0, ',', '.') }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="bg-green-50 rounded-lg p-4">
        <div class="flex items-center">
          ...
          <div class="ml-3">
            <p class="text-sm font-medium text-green-800">Total Tahun Ini</p>
            <p class="text-lg font-semibold text-green-900">Rp {{ number_format($summary['this_year'] ?? 0, 0, ',', '.') }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="bg-purple-50 rounded-lg p-4">
        <div class="flex items-center">
          ...
          <div class="ml-3">
            <p class="text-sm font-medium text-purple-800">Pending Payment</p>
            <p class="text-lg font-semibold text-purple-900">Rp {{ number_format($summary['pending'] ?? 0, 0, ',', '.') }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="bg-gray-50 rounded-lg p-4">
        <div class="flex items-center">
          ...
          <div class="ml-3">
            <p class="text-sm font-medium text-gray-800">Total All Time</p>
            <p class="text-lg font-semibold text-gray-900">Rp {{ number_format($summary['all_time'] ?? 0, 0, ',', '.') }}</p>
          </div>
        </div>
      </div>
    </div>
