            ...
            <div class="flex items-center">
                ...
                <div class="ml-5 w-0 flex-1">
                    <dl>
                        ...
                        <dd class="text-2xl font-bold text-gray-900">{{ \App\Models\Motor::count() }}</dd>
                    </dl>
                </div>
            </div>
        </div>
            ...
            <div class="flex items-center">
                ...
                <div class="ml-5 w-0 flex-1">
                    <dl>
                        ...
                        <dd class="text-2xl font-bold text-gray-900">{{ \App\Models\Motor::where('status', \App\Enums\MotorStatus::PENDING)->count() }}</dd>
                    </dl>
                </div>
            </div>
        </div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div class="flex items-center space-x-4">
                    <div class="flex-shrink-0">
                        <div class="h-10 w-10 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center">
                            <span class="text-sm font-medium text-white">{{ substr($user->nama, 0, 1) }}</span>
                        </div>
                    </div>
    ...
    <div class="bg-white shadow-lg rounded-xl border border-gray-100">
        ...
        <div class="p-6">
            <div class="space-y-4">
                @if(\App\Models\Motor::where('status', \App\Enums\MotorStatus::PENDING)->count() > 0)
                @foreach(\App\Models\Motor::where('status', \App\Enums\MotorStatus::PENDING)->latest()->take(5)->get() as $motor)
                <div class="flex items-center space-x-4">
                    <div class="flex-shrink-0">
                        @if($motor->photo)
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-medium text-gray-900 truncate">{{ $motor->merk }} {{ $motor->model }}</p>
                        <p class="text-sm text-gray-500 truncate">{{ $motor->no_plat }}</p>
                    </div>
                    <div class="flex-shrink-0">
