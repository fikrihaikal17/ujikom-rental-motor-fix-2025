  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="space-y-8">
      ...
      <div class="bg-white shadow-sm rounded-lg overflow-hidden">
        ...
        <form method="POST" action="{{ route('owner.profile.update') }}" class="px-6 py-6">
          @csrf
          @method('PUT')
            ...
            <div>
              ...
              <div class="relative">
                <input 
                  ...
                  name="no_telepon" 
                  id="no_telepon" 
                  value="{{ old('no_telepon', $user->no_tlpn) }}" 
                  placeholder="08xxxxxxxxxx"
                  class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('no_telepon') border-red-300 focus:ring-red-500 focus:border-red-500 @enderror"
                >
                <div class="absolute inset-y-0 right-0 flex items-center pr-3">
                  <svg class="h-5 w-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="px-6 py-6">
          <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
            <div class="flex items-center p-4 bg-gradient-to-r from-blue-50 to-blue-100 rounded-lg border border-blue-200">
              ...
              <div class="ml-4">
                <dt class="text-sm font-medium text-blue-700">Total Motor</dt>
                <dd class="text-2xl font-bold text-blue-900">{{ Auth::user()->motors()->count() }}</dd>
                <p class="text-xs text-blue-600">Motor terdaftar</p>
              </div>
            </div>
            ...
            <div class="flex items-center p-4 bg-gradient-to-r from-green-50 to-green-100 rounded-lg border border-green-200">
              ...
              <div class="ml-4">
                <dt class="text-sm font-medium text-green-700">Motor Tersedia</dt>
                <dd class="text-2xl font-bold text-green-900">{{ Auth::user()->motors()->where('status', \App\Enums\MotorStatus::AVAILABLE)->count() }}</dd>
                <p class="text-xs text-green-600">Siap disewa</p>
              </div>
            </div>
            ...
            <div class="flex items-center p-4 bg-gradient-to-r from-purple-50 to-purple-100 rounded-lg border border-purple-200">
              ...
              <div class="ml-4">
                ...
                <dd class="text-2xl font-bold text-purple-900">{{ Auth::user()->created_at->format('M Y') }}</dd>
                <p class="text-xs text-purple-600">{{ Auth::user()->created_at->diffForHumans() }}</p>
              </div>
            </div>
          </div>
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <div class="text-center p-4 bg-gray-50 rounded-lg">
                <div class="text-2xl font-bold text-gray-900">
                  {{ Auth::user()->motors()->whereHas('penyewaans')->count() }}
                </div>
                <div class="text-sm text-gray-600">Motor Pernah Disewa</div>
              </div>
              
              @php
                $totalRentals = 0;
                foreach(Auth::user()->motors as $motor) {
              ...
              <div class="text-center p-4 bg-gray-50 rounded-lg">
                <div class="text-2xl font-bold text-green-600">
                  Rp {{ number_format(Auth::user()->bagiHasils()->sum('bagi_hasil_pemilik'), 0, ',', '.') }}
                </div>
              ...
              <div class="text-center p-4 bg-gray-50 rounded-lg">
                <div class="text-2xl font-bold text-blue-600">
                  {{ Auth::user()->motors()->where('status', \App\Enums\MotorStatus::RENTED)->count() }}
                </div>
